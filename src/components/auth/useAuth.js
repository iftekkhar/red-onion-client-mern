...
const UserProvider = (props) => {
  ...
  const login = async (email, password) => {
    try {
      ...
      const userInfo = await axios.get(`https://red-onion-ia.herokuapp.com/api/v1/users/email/${email}`);
  ...
  const registerUserWithEmailPassword = async (email, password, name) => {
    try {
      ...
      const createUser = await axios.post('https://red-onion-ia.herokuapp.com/api/v1/users', { email, displayName: name });
    ...
    firebase.auth().onAuthStateChanged(function (loginUser) {
      ...
      if (loginUser) {
        async function getUserProfile(email) {
          const response = await axios.get(`https://red-onion-ia.herokuapp.com/api/v1/users/email/${email}`);
  ...
  useEffect(() => {
    async function getCarts() {
      if (user) {
        try {
          const response = await axios.get(`https://red-onion-ia.herokuapp.com/api/v1/carts/${user._id}`);
  ...
  const calculateQuantity = (item, event) => {
    ...
    if (product.quantity === 0) {
      ...
      // delete product
      axios.delete(`https://red-onion-ia.herokuapp.com/api/v1/carts/${item._id}?user=${user._id}`)
      setCart(updateProduct)
    } else {
      // update product
      axios.put(`https://red-onion-ia.herokuapp.com/api/v1/carts/${item._id}`, { quantity: product.quantity, user: user._id })
  ...
  function addCatDatabase(item, quantity, user) {
    ...
    axios.post('https://red-onion-ia.herokuapp.com/api/v1/carts', cartData);
  }
